...
- Large emphasis on continuous delivery:
  ...
  - ability to make reliable updates - is the essence of long-term sustainability for your project
...
Distributed/Scale knowledge
 ...
 - don't reform/remove/rewrite system without understanding the context
    ...
    - great decisions depend on understanding the rational behind previous decisions
...
- don't be everyone's friend - but be humble and respectful
...
- do lose the ego - encourage inquiry, seek constructive feedback apologize, acknowledge your mistakes,
...
- do be honest - i'm not allowed to tell you, I want you to know that you are doing something wrong, but I'm committed to helping you improve
- do track happiness - make sure they get recognition, shitty tasks are spread evenly, no one works long hours
...
### Research advice
 - what results are you expecting and why? - fight bias, ad-hoc explanations of results
 - if data supports your expected results, what action will be taken? - if no action/no point of measuring
 - if we get a negative results, will appropriate action be taken? - if negative results don't change a decision (there are other factors/inputs), it might not be worth measuring
 - who is going to take action on the results and when? - what kind of evidence is expected and what is the time frame
...
Simpler solutions are always preferable
  ...
  - easier to change
  - complex functionality might never be needed - YAGNI - or not needed in the way it was implemented
...
Test maintainability - (usefulness)
  - test public api/exported methods (user perspective), not private/inner methods/helper methods
...
Test doubles - Mocks - stubbing & interaction testing
  ...
  - interaction testing - (mocks) how a function is called - verify() - mocking frameworks
    ...
    - should be only used to stub minimum amount of functions, and only those that relate to the test assertion

**Hyrum's Law** - with enough users, all observable behavior will be depended on (not just your API contract)

Deprecation
  - code is a liability, and the functionality it provides is an asset
  - plan for deprecation
    - design successor systems with a migration path from older systems in mind
    - can parts be easily replaced? can clients easily migrate?
...
Build system: Flexibility vs Determinism->Parallelism
Task-based build systems
  - define tasks and dependencies between tasks
  - imperative buildfiles, arbitrary tasks
  - flexible, more control for the developer, but non deterministic build system cannot track/parallelize steps
...
Martin Fowler: “The biggest risk to any software effort is that you end up building something that isn’t useful.
  ...
  - release deadline - late features go on the next train
...
No software is perfect - and that's okay - as long as hits your set targets
  ...
  - core, primary functionalities, risk mitigations tested
...
Compute-As-A-Service
 ...
 - different use cases: batch vs serving
...
API/Service design trade-off
  ...
  - generic API, easier to manage, coherence - vs - usage inconvenience
